...
def main():
    ...
    if st.button(f"🚀 Lancer la recherche pour {collection_a_chercher_rennes}"):
        ...
        if st.session_state.get('last_result_df') is not None:
            ...
            # 🧠 Étape 1 : injection OpenAlex pour toutes les notices (avant filtrage)
            ...
            # 🧹 Étape 2 : filtrage des publications hors HAL (pour export)
        ...
        # ---------------------------------------------------
        # 🧩 Étape 3 : génération ZIP + téléchargement (même niveau que le bloc précédent)
        # ---------------------------------------------------
        if st.button(f"⬇️ Télécharger le fichier ZIP des XML HAL ({len(pubs_to_export)})", key=f"dlzip_{last_collection}"):

            # Normalisation stricte des structures
